...
def get_spherical_components(tens,CS,threshold,keep_cols,all_sym):
    ...
    if (len(keep_cols[-1]) > 0):
        degen = [1 + 2*keep_cols[-1][i][-1] for i in xrange(len(keep_cols[-1]))]
    else:
        degen = [1]
        return [np.array(tens).astype(float),[1],None,[1]]
    cumulative_degen = [sum(degen[:i]) for i in xrange(1,len(degen)+1)]

    # Get CR matrix
    CR = complex_to_real_transformation(degen)

    all_tens_sphr = [ [] for i in xrange(len(degen))]
...
def convert_spherical_to_cartesian(outvec,degen,size,CR,CS,keep_cols,keep_list,lin_dep_list,sym_list):
    ...
    full_degen = [2*keep_cols[-1][i][-1] + 1 for i in xrange(len(keep_cols[-1]))]

    full_outvec = [np.zeros((size,full_degen[i]),dtype=complex) for i in xrange(len(full_degen))]
    ...
    for i in xrange(len(lin_dep_list)):
    ...
    for i in xrange(len(outvec[0])/degen[0]):
        full_outvecs = []
        for j in xrange(len(full_degen)):
            if (full_degen[j]==1):
                full_outvecs.append(full_outvec[j][i])
            else:
                for k in xrange(len(full_outvec[j][i])):
...
def get_cartesian_to_spherical(rank):
    ...
    if rank > 1:
        for i in xrange(2,rank+1):
            ...
            # Get list of columns
            col = [[1]]
            for rn in xrange(2,i+1):
                col = [col[k] + [j] for k in xrange(len(col)) for j in xrange(rn+1)]
            keep_col = []
            for cc in xrange(len(col)):
                ...
                dif_list = np.abs([col_list[n] - col_list[n-1] for n in xrange(1,len(col_list))])
                sum_list = [col_list[n] + col_list[n-1] for n in xrange(1,len(col_list))]
                if ((min(sum_list) > 0) and (max(dif_list) <= 1)):
                    keep_col.append(col[cc])
            # Now include m values
            keep_cols.append(keep_col)
            col = []
            for cc in xrange(len(keep_col)):
                jj = keep_col[cc][-1]
                for m in xrange(2*jj + 1):
                    col.append(keep_col[cc] + [m-jj])
            if (len(col) != 3**i):
                print "ERROR: the number of columns is incorrect!"
                sys.exit(0)
            # Find rows
            row = [map(int,np.base_repr(rr,3).rjust(i,'0')) for rr in xrange(3**i)]
            for cc in xrange(3**i):
                for rr in xrange(3**i):
    ...
    if rank > 2:
        for i in xrange(3,rank+1):
            sym = []
            for j in xrange(len(keep_cols[i])):
