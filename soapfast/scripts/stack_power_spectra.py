...
all_ps=[np.load("PS_output_" + str(i+1) + ".npy") for i in xrange(nrun)]
all_natom=[np.load("PS_output_" + str(i+1) + "_natoms.npy") for i in xrange(nrun)]

# Put power spectra together
npoints = sum([np.shape(all_ps[i])[0] for i in xrange(nrun)])
natmax  = max([np.shape(all_ps[i])[1] for i in xrange(nrun)])
nfeat   = np.shape(all_ps[0])[-1]
if (len(np.shape(all_ps[0]))==3):
    degen=1
    intermediate_ps = [np.zeros((len(all_ps[i]),natmax,nfeat),float) for i in xrange(nrun)]
else:
    degen=len(all_ps[0][0,0])
    intermediate_ps = [np.zeros((len(all_ps[i]),natmax,degen,nfeat),float) for i in xrange(nrun)]
