...
def get_power_spectrum(lam,frames,nmax=8,lmax=6,rc=4.0,sg=0.3,ncut=-1,cw=1.0,periodic=False,outfile='',subset=['NO',None],initial=-1,sparsefile='',sparse_options=[''],cen=[],spec=[],atomic=[False,None],all_radial=[1.0,0.0,0.0],useall=False,xyz_slice=[],verbose=True,get_imag=False):
    ...
    npoints   = len(frames)
    all_names = np.array([frames[i].get_chemical_symbols() for i in xrange(npoints)])
    natmax    = len(max(all_names, key=len))
    coords    = [frames[i].get_positions() for i in xrange(npoints)]

    # Periodic or aperiodic?
    if periodic == True:
        cell = [frames[i].get_cell() for i in xrange(npoints)]
    else:
    ...
    nneighmax = [np.array([len(list(np.where(np.array(all_names[i][:nat[i]]) == chemical_symbols[ispe])[0])) for ispe in xrange(nsmax)]) for i in xrange(npoints)]
    ...
    # Get list of species
    if (spec != []):
        all_species = list([atomic_numbers[spec[i]] for i in xrange(len(spec))])
    ...
    [orthomatrix,sigma] = utils.PS_utils.setup_orthomatrix(nmax,rc)
    ...
    [power,featsize] = utils.PS_utils.compute_power_spectrum(nat,nneighmax,natmax,lam,lmax,npoints,nspecies,nnmax,nmax,llmax,lvalues,centers,all_indexes,all_species,coords,cell,rc,cw,sigma,sg,orthomatrix,sparse_options,all_radial,ncen,useall,verbose)
    if (verbose):
    ...
    if sparsefile != '':
        ...
        [psparse,sparse_details] = utils.PS_utils.FPS_sparsify(power.reshape((2*lam+1)*npoints*natmax,featsize),featsize,ncut,initial)
        featsize = len(psparse[0])
        if (verbose):
    ...
    for i in xrange(npoints):
        ...
        for j in xrange(len(reorder_list)):
    ...
    if (subset[0] == 'NO'):
        ...
        if (atomic[0] == True):
            tot_atoms = sum([len(all_names[i]) for i in xrange(npoints)])
            tot_atoms = sum(nat)
            if (lam==0):
...
def main():
    ...
    args = utils.parsing.add_command_line_arguments_PS("Calculate power spectrum")
    [nmax,lmax,rcut,sig,cen,spec,cweight,lam,periodic,ncut,sparsefile,frames,subset,sparse_options,outfile,initial,atomic,all_radial,useall,xyz_slice,get_imag] = utils.parsing.set_variable_values_PS(args)
