...
def get_kernel(PS,scale=[],PS0=[None,None],zeta=1,output='',use_hermiticity=False,verbose=True):
    ...
    if (scale==[]):
        for j in xrange(2):
            scale.append(np.array([1 for i in xrange(npoints[j])]))
    ...
    if lam == 0:
        ...
        if zeta == 1: 
            ...
            poweravg = [np.einsum('ab,a->ab',np.sum(PS[i],axis=1),1.0/scale[i]) for i in xrange(2)]
            # get kernel    
        ...
        PS = [np.einsum('cdbe,c->cdbe',PS[i],1.0/scale[i]) for i in xrange(2)]

        if zeta == 1: 

            # compute power spectrum average
            poweravg = [np.sum(PS[i],axis=1) for i in xrange(2)]
            # get kernel
