...
def main():
    ...
    [reg,fractrain,tens,kernels,sel,rdm,rank,nat,peratom,prediction,weights,sparsify,mode,threshold,jitter] = utils.parsing.set_variable_values_learn(args)
    ...
    if (args.spherical == False):
        ...
        if (sparsify == None):
            ...
            for l in xrange(len(degen)):
                ...
                [ov, tv, na] = utils.sagpr_utils.do_sagpr_spherical(kernel[l],spherical_tensor[l],reg[l],rank_str=str_rank,nat=nat,fractrain=fractrain,rdm=rdm,sel=sel,peratom=peratom,prediction=prediction,get_meantrain=True,mode=mode,wfile=weights,fnames=[args.features,kernels[l]],jitter=jitter[l])
                outvec.append(ov)
                tstvec.append(tv)
            nattest = na
            ...
            if (prediction):
                ...
                testcart  = utils.regression_utils.convert_spherical_to_cartesian(tstvec,degen,ns,CR,CS,keep_cols,keep_list,lin_dep_list,sym_list)
        ...
        # Do full Cartesian regression, with environmental sparsification
            ...
            nN = 0
            nM = 0
            for k in xrange(len(sparsify)/2):
            ...
            if (prediction):
                if (len(sel)==1):
                    ...
                    training_set = range(sel[0],sel[1])
                elif (rdm!=0):
                    training_set = range(nN)
                ...
                test_set = np.setdiff1d(range(nN),training_set)
                ktr = []
                kte = []
                for k in xrange(len(sparsify)/2):
                    ktr.append(np.array([[kernel[2*k][i,j] for j in xrange(len(kernel[2*k][0]))] for i in training_set]).astype(float))
                    kte.append(np.array([[kernel[2*k][i,j] for j in xrange(len(kernel[2*k][0]))] for i in test_set]).astype(float))
                    kernel[2*k] = ktr[-1]
            ...
            meantrain = [0 for i in xrange(len(degen))]
            for i in xrange(len(degen)):
                if degen[i]==1:
            ...
            for l in xrange(len(degen)):
                lval = keep_list[l][-1]
                utils.sagpr_utils.do_sagpr_spherical(sparse_kernel[l],spherical_tensor[l],reg[l],rank_str=''.join(map(str,keep_list[l][1:])),nat=nat,fractrain=fractrain,rdm=rdm,sel=sel,peratom=peratom,prediction=False,reg_matr=reg_matr[l],get_meantrain=False,mode=mode,wfile=weights,fnames=[args.features,sparsify[l]],jitter=jitter[l])
    
            # Deal with meantrain: put this into the weights so that we can do prediction
            for i in xrange(len(degen)):
                str_rank = ''.join(map(str,keep_list[i][1:]))
                if (degen[i]==1):
                    wts = np.load(weights + "_" + str_rank + ".npy")
                    wts[5] = meantrain[i]
            ...
            if (prediction):
                    ...
                    if (degen[i]==1):
                        ...
                        if peratom:
                            ...
                            for j in xrange(len(pred[i])):
                                print >> corrfile, pte[i][j]*nattest[j],"  ",pred[i][j]*nattest[j],"  ",nattest[j]
                            corrfile.close()
                        else:
                            corrfile = open("prediction_L" + str_rank + ".txt","w")
                            for j in xrange(len(pred)):
                                print >> corrfile, pte[i][j],"  ",pred[i][j]
                            corrfile.close()
                        # Accumulate errors
                        intrins_dev = np.std(ptr[i])**2
                        abs_error = 0.0
                        for j in xrange(len(pte[i])):
                        ...
                        if peratom:
                            ...
                            for j in xrange(len(prediction)):
                                print >> corrfile, ' '.join(str(e) for e in list(np.array(comparison[j])*nattest[j])),"  ",' '.join(str(e) for e in list(np.array(prediction[j])*nattest[j])),"  ",nattest[j]
                            corrfile.close()
                        else:
                            corrfile = open("prediction_L" + str_rank + ".txt","w")
                            for j in xrange(len(prediction)):
                                print >> corrfile, ' '.join(str(e) for e in list(np.array(comparison[j]))),"  ",' '.join(str(e) for e in list(np.array(prediction[j])))
                            corrfile.close()
                        # Accumulate errors
                        intrins_dev=0.0
                        abs_error=0.0
                        training = ptr[i].reshape(len(training_set),degen[i])
                        for j in xrange(len(training)):
                            intrins_dev += np.linalg.norm(training[j])**2
                        for j in xrange(len(comparison)):
                ...
                ns = len(prediction)
                predcart = utils.regression_utils.convert_spherical_to_cartesian(pred,degen,ns,CR,CS,keep_cols,keep_list,lin_dep_list,sym_list)
                testcart = utils.regression_utils.convert_spherical_to_cartesian(pte ,degen,ns,CR,CS,keep_cols,keep_list,lin_dep_list,sym_list)
        ...
        if (sparsify == None):
            ...
            utils.sagpr_utils.do_sagpr_spherical(kernel,spherical_tensor,reg,rank_str=str(rank),nat=nat,fractrain=fractrain,rdm=rdm,sel=sel,peratom=peratom,prediction=prediction,get_meantrain=True,mode=mode,wfile=weights,fnames=[args.features,kernels],jitter=jitter[0])
        ...
        # Do spherical regression, with environmental sparsification
            ...
            if (prediction):
                ...
                test_set = np.setdiff1d(range(nN),training_set)
                ktr = np.array([[kernel[0][i,j] for j in xrange(len(kernel[0][0]))] for i in training_set]).astype(float)
                kte = np.array([[kernel[0][i,j] for j in xrange(len(kernel[0][0]))] for i in test_set]).astype(float)
                kernel[0] = ktr
            ...
            if (int_rank == 0):
                spherical_tensor = np.array([spherical_tensor[i] for i in xrange(len(spherical_tensor))])
    
            # Put tensor into float form
            spherical_tensor = np.split(spherical_tensor,len(spherical_tensor)/(2*int_rank+1))
    
            # Pass these matrices to the SA-GPR routine
            utils.sagpr_utils.do_sagpr_spherical(sparse_kernel,spherical_tensor,reg,rank_str=str(rank),nat=nat,fractrain=fractrain,rdm=rdm,sel=sel,peratom=peratom,prediction=False,reg_matr=reg_matr,get_meantrain=False,mode=mode,wfile=weights,fnames=[args.features,sparsify],jitter=jitter[0])
                ...
                if (int_rank==0):
                    ...
                    if peratom:
                        ...
                        for i in xrange(len(pred)):
                            print >> corrfile, pte[i]*nattest[i],"  ",pred[i]*nattest[i],"  ",nattest[i]
                    else:
                        corrfile = open("prediction_L" + str(int_rank) + ".txt","w")
                        for i in xrange(len(pred)):
                            print >> corrfile, pte[i],"  ",pred[i]
                    # Accumulate errors
                    intrins_dev = np.std(ptr)**2
                    abs_error = 0.0
                    for i in xrange(len(pte)):
                    ...
                    if peratom:
                        corrfile = open("prediction_L" + str(int_rank) + ".txt","w")
                        for i in xrange(len(pred)):
                            print >> corrfile, ' '.join(str(e) for e in list(np.array(pte[i])*nattest[i])),"  ",' '.join(str(e) for e in list(np.array(pred[i])*nattest[i])),"  ",nattest[i]
                    else:
                        corrfile = open("prediction_L" + str(int_rank) + ".txt","w")
                        for i in xrange(len(pred)):
                            print >> corrfile, ' '.join(str(e) for e in list(np.array(pte[i]))),"  ",' '.join(str(e) for e in list(np.array(pred[i])))
                    intrins_dev = np.std(ptr)**2
                    intrins_dev = 0.0
                    abs_error = 0.0
