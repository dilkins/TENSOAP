...
get_atomic_power_spectrum.py -lm 0 -p PS0.npy -f ../coords.xyz -o PS0_atomic > /dev/null
get_atomic_power_spectrum.py -lm 2 -p PS2.npy -f ../coords.xyz -o PS2_atomic > /dev/null
...
# build L=0 kernels
sagpr_get_kernel -lm 0 -z 2 -ps PS0.npy PS0_atomic_sparse.npy -o KER0_NM -s PS0_natoms.npy NONE > /dev/null
sagpr_get_kernel -lm 0 -z 2 -ps PS0_atomic_sparse.npy -o KER0_MM -s NONE > /dev/null

# build L=2 kernels
sagpr_get_kernel -lm 2 -z 2 -ps PS2.npy PS2_atomic_sparse.npy -ps0 PS0.npy PS0_atomic_sparse.npy -o KER2_NM -s PS2_natoms.npy NONE > /dev/null
sagpr_get_kernel -lm 2 -z 2 -ps PS2_atomic_sparse.npy -ps0 PS0_atomic_sparse.npy -o KER2_MM -s NONE > /dev/null
...
python -c 'import numpy as np;wC = np.load("wt_cart_0.npy");wS = np.load("wt_sphr_0.npy");print 100 * np.linalg.norm(wC[4]-wS[4]) / np.linalg.norm(wC[4]),"%"'
python -c 'import numpy as np;wC = np.load("wt_cart_2.npy");wS = np.load("wt_sphr_2.npy");print 100 * np.linalg.norm(wC[4]-wS[4]) / np.linalg.norm(wC[4]),"%"'
